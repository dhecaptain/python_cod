...
def deposit():
    while True:
        ...
        if amount.isdigit ():
            ...
            print('enter a digit kindly ')
    return amount

def get_number_of_lines():
    while True:
        lines = input( " enter number of line you wish to bet on(1-" + str(MAX_LINES) + ")? ")
        if lines.isdigit ():
            lines = int(lines)
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
                print (f"you do not have enough money to bet on, your current balance is ${balance} ")
        else:
            break
    print(f"your betting ${bet} on {lines} lines. Total bet equals: ${total_bet}")
